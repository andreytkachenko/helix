...
impl TerminalView {
    ...
    async fn handle_key_event(
        ...
        id: TerminalId,
        key: input::KeyEvent,
    ) -> Result<bool, helix_vte::error::Error> {
        Ok(if let Some(s) = resolve_key_event(key) {
    ...
    async fn handle_input_event_async(
        ...
        Ok(match event {
            input::Event::FocusGained => false,
            input::Event::FocusLost => false,
            ...
            input::Event::IdleTimeout => false,
        })
    }

    pub fn handle_input_event(&mut self, event: &input::Event) -> bool {
        if let Some(id) = self.active_term {
            helix_lsp::block_on(self.handle_input_event_async(id, event)).unwrap_or(false)
        } else {
            false
        }
    }
    ...
    async fn handle_mouse_event(
        ...
        let res = if let Some(mut term) = self.get_term_mut(id) {
            match evt.kind {
